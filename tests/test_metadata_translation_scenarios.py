...
async def test_execute_success(mock_repo, mock_translator, sample_catalog):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        request = TranslateMetadataRequest(
            path=path, source_locale="en-US", target_locales=["de-DE"], dry_run=False
        )
...
async def test_execute_character_limit_warn(mock_repo, mock_translator, sample_catalog):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (
            len(written_catalog.get_locale("de-DE").get_field(MetadataFieldType.NAME).content) == 35
        )
...
async def test_execute_character_limit_truncate(mock_repo, mock_translator, sample_catalog):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (
            len(written_catalog.get_locale("de-DE").get_field(MetadataFieldType.NAME).content) == 30
        )
...
async def test_execute_overwrite(mock_repo, mock_translator, sample_catalog):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        written_catalog = mock_repo.write.call_args[0][0]
        assert (
            written_catalog.get_locale("de-DE").get_field(MetadataFieldType.NAME).content
            == "New German Translation"
        )


@pytest.mark.asyncio
async def test_keywords_translated_separately_from_batch(mock_repo, mock_translator):
    """Keywords get their own ASO-aware API call and never go through the batch prompt."""
    catalog = MetadataCatalog(source_locale="en-US")
    en_locale = LocaleMetadata(locale="en-US")
    en_locale.set_field(MetadataFieldType.NAME, "TaskMaster")
    en_locale.set_field(MetadataFieldType.SUBTITLE, "Plan your day")
    en_locale.set_field(
        MetadataFieldType.DESCRIPTION, "A productivity app for managing daily tasks and goals."
    )
    en_locale.set_field(MetadataFieldType.KEYWORDS, "todo,task,planner,reminder,productivity")
    catalog.locales["en-US"] = en_locale
    mock_repo.read.return_value = catalog
    ...
    async def mock_call(prompt):
        ...
        if "<<ITEM_1>>" in prompt:
            # Batch path: must not contain the keywords field
            return (
                "<<ITEM_1>>Aufgabenplaner<</ITEM_1>>\n"
                "<<ITEM_2>>Plane deinen Tag<</ITEM_2>>\n"
                "<<ITEM_3>>Eine Produktivitäts-App.<</ITEM_3>>"
            )
        # Keywords path
        return "aufgaben,planer,checkliste,erinnerung,tagesplan"
    ...
    assert result.saved is True
    # Two API calls: one for keywords, one for the batch of the other 3 fields
    assert mock_translator._call_api.call_count == 2

    keyword_prompts = [p for p in captured_prompts if "<<ITEM_1>>" not in p]
    batch_prompts = [p for p in captured_prompts if "<<ITEM_1>>" in p]
    assert len(keyword_prompts) == 1
    assert len(batch_prompts) == 1

    keyword_prompt = keyword_prompts[0]
    # ASO framing
    assert "ASO" in keyword_prompt
    # AppContext is injected
    assert "TaskMaster" in keyword_prompt
    assert "APP CONTEXT" in keyword_prompt
    # Source keywords still shown for reference
    assert "todo,task,planner,reminder,productivity" in keyword_prompt

    # Keywords must NOT leak into the batch prompt
    assert "todo,task,planner" not in batch_prompts[0]
    assert "KEYWORDS" not in batch_prompts[0]

    written_catalog = mock_repo.write.call_args[0][0]
    de_keywords = written_catalog.get_locale("de-DE").get_field(MetadataFieldType.KEYWORDS)
    assert de_keywords.content == "aufgaben,planer,checkliste,erinnerung,tagesplan"


def test_build_keywords_prompt_without_app_context_omits_context_block():
    """build_keywords_prompt should still work (no APP CONTEXT block) when app_context is None."""
    from localizerx.translator.metadata_prompts import build_keywords_prompt
    ...
    assert "todo,task,planner" in prompt
